...
void FillArrayWithColsSum(short x[3][3], short Arr[3], short row, short cols)
{
    for (short i = 0; i < row; i++)
    {
        Arr[i] = GetColsSum(x, row, i);
    }
}

void PrintSumOfColsInMatrix(short x[3][3], short Arr[3], short Row, short Cols)
{
    for (int i = 0; i < Row; i++)
    {
        cout << "The sum of Col " << i + 1 << " = " <<Arr[i] << endl;
    }
}
...
int main()
{
    ...
    FillArrayWithColsSum(x, Arr, 3, 3);

    PrintSumOfColsInMatrix(x, Arr, 3, 3);
